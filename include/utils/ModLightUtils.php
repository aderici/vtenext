...
class ModLightUtils extends SDKExtendableUniqueClass {
	...
	function addTableField($blockid, $addfieldno, $properties, $forceSuffix = '') { // crmv@198024
		...
		if ($result && $adb->num_rows($result) > 0) {
			...
			$module->setDefaultSharing('Private', 2); // crmv@193317
	...
	function deleteTableField($editfieldno, $log=true) {
		...
		// drop tables
		if (Vtiger_Utils::CheckTable("{$table_prefix}_modlight{$modulelightid}")) $adb->query("drop table {$table_prefix}_modlight{$modulelightid}");
		if (Vtiger_Utils::CheckTable("{$table_prefix}_modlight{$modulelightid}cf")) $adb->query("drop table {$table_prefix}_modlight{$modulelightid}cf");
