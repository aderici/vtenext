...
function isModuleInstalled($module) {	/* crmv@22700	crmv@27624	crmv@25671	crmv@47905bis */
	...
	if (in_array($module, array('Administration','com_vtiger_workflow','CustomView','Help','Import','PickList','Picklistmulti','Settings','Yahoo'))) return true;
	...
	if (is_array($columns_to_check)) {
		foreach ($columns_to_check as $_to_split){
			...
			if (in_array($uitype, array(50,51,52,54)) && $fieldname != 'smownerid') {
				if (stripos($query," {$table_prefix}_users_fld_{$fieldid}") === false) {
				...
				if (stripos($query," {$table_prefix}_groups_fld_{$fieldid}") === false) {
...
function requestFromPortal() {
	return (
		preg_match('/vteservice.php$/', $_SERVER['SCRIPT_NAME']) > 0 ||
		preg_match('/vtigerservice.php$/', $_SERVER['SCRIPT_NAME']) > 0 // kept for compatibility, will be removed in the future
	);
}
