...
class Vtiger_MailRecord {
		...
		for ($i=0; $i<count($tabChaine); $i++) {
			switch (strtoupper($tabChaine[$i]->charset)) { // convert charset to uppercase
				...
				case 'DEFAULT':
					...
					break;
				default:
					if (in_array(strtoupper($tabChaine[$i]->charset),upperListEncode())) {
	...
	function __parseHeader($imap, $messageid) {
		...
		// crmv@170905
		Vtiger_MailScanner::performanceLog('Fetching header [IMAP-MSG-ID=' . $messageid . ']', 'fetch_header');
		
		$messageid = imap_msgno($imap,$this->_xuid); //crmv@70040
		$mailheader = imap_headerinfo($imap, $messageid);
		
		Vtiger_MailScanner::performanceLog('Fetched header in {tac}', 'fetch_header', true);
		
		$base = log($mailheader->Size) / log(1024);
		$suffix = array("", "k", "M", "G", "T");
		$suffix = $suffix[floor($base)];
		Vtiger_MailScanner::performanceLog('Size: ' . round(pow(1024, $base - floor($base)), 2) . $suffix);
		// crmv@170905e
	...
	function __parseBody($imap, $messageid) {
		// crmv@170905
		Vtiger_MailScanner::performanceLog('Fetching body [IMAP-MSG-ID=' . $messageid . ']', 'fetch_body');
		...
		$errors = imap_errors();
		if (!empty($errors)) {
			Vtiger_MailScanner::performanceLog('Errors: ' . print_r($errors, true));
		}
		
		Vtiger_MailScanner::performanceLog('Fetched body in {tac}', 'fetch_body', true);
		// crmv@170905e
		...
		if($this->_attachments) {
			...
			Vtiger_MailScanner::performanceLog('Attachments: ' . count($this->_attachments)); // crmv@170905
		}
