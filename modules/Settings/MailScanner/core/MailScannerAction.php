	...
	function apply($mailscanner, $mailrecord, $mailscannerrule, $matchresult) {
		...
		if($this->actiontype == 'CREATE') {
			if($this->module == 'HelpDesk') {
				Vtiger_MailScanner::performanceLog('Creating ticket...', 'create_ticket'); // crmv@170905
				$returnid = $this->__CreateTicket($mailscanner, $mailrecord);
				Vtiger_MailScanner::performanceLog("Ticket [$returnid] created and rules applied in {tac}", 'create_ticket', true); // crmv@170905
			}
		} else if($this->actiontype == 'LINK') {
			Vtiger_MailScanner::performanceLog('Linking record...', 'link_record'); // crmv@170905
			$returnid = $this->__LinkToRecord($mailscanner, $mailrecord);
			Vtiger_MailScanner::performanceLog("Record [$returnid] linked and rules applied in {tac}", 'link_record', true); // crmv@170905
		} else if($this->actiontype == 'UPDATE') {
			if($this->module == 'HelpDesk') {
				Vtiger_MailScanner::performanceLog('Updating ticket...', 'update_ticket'); // crmv@170905
				...
				Vtiger_MailScanner::performanceLog("Ticket [$returnid] updated and rules applied in {tac}", 'update_ticket', true); // crmv@170905
			}
		//crmv@27618
