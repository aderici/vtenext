	...
	function performScanNow() {
		...
		foreach($folders as $lookAtFolder) {
			...
			foreach($mailsearch as $messageid) {
				...
				Vtiger_MailScanner::performanceLog("START_FETCH", false, false, array('messageid' => $messageid)); // crmv@170905
				...
				if($this->isMessageScanned($mailrecord, $lookAtFolder)) {
					...
					Vtiger_MailScanner::performanceLog("Message already scanned [$mailrecord->_subject], IGNORING..."); // crmv@170905
					Vtiger_MailScanner::performanceLog("END_FETCH", false, false, array('messageid' => $messageid, 'processed' => false)); // crmv@170905
					unset($mailrecord);
					continue;
				}
				...
				Vtiger_MailScanner::performanceLog("Applying rules for mail..."); // crmv@170905
				...
				if ($move_to != '') {
					Vtiger_MailScanner::performanceLog("Moving message to folder $move_to...", 'move_message'); // crmv@170905
					
					//$mailbox->moveMessage($messageid, $move_to);
					$mailbox->moveMessage($messageid, $move_to,true); //crmv@70040
					
					Vtiger_MailScanner::performanceLog("Message moved to folder $move_to in {tac}", 'move_message', true); // crmv@170905
				}
				//crmv@2043me
				
				Vtiger_MailScanner::performanceLog("FETCH_DETAILS", false, false, array('mailrecord' => $mailrecord)); // crmv@170905
				Vtiger_MailScanner::performanceLog("END_FETCH", false, false, array('messageid' => $messageid, 'processed' => true)); // crmv@170905
