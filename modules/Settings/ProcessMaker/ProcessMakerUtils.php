...
class ProcessMakerUtils extends SDKExtendableUniqueClass {
	...
	var $actionTypes = array(
		...
		'Cycle' => array(
			...
			'actions'=>array('Email','Create','ModNotification','InsertTableRow','DeleteTableRow','InsertProductRow','DeleteProductRow') //crmv@183346 crmv@195745
		),
		//crmv@112297
...
//	}
	...
	function save($data, $id=null, $overwrite=false, $pending_changes=true) {
		global $adb, $table_prefix, $default_charset, $current_user;	//crmv@132240 crmv@193096
		...
		if (empty($id) || $overwrite) {
			...
			if (empty($id)) {
				...
				if (in_array('creatorid', $cols)) $values['creatorid'] = $current_user->id;
	...
	function setRecordInvolved($id,$elementid,$text,$type,$module,$action,$start=0) {
		...
		if ($result && $adb->num_rows($result) > 0) {
			...
			$adb->pquery("insert into {$table_prefix}_processmaker_metarec values (?,?,?,?,?,?,?,?)", array($metarecid,$id,$elementid,html_entity_decode($text,ENT_QUOTES,$default_charset),$type,$module,$action,$start));	//crmv@132240
		}
	}
	...
	function getSystemVersion4Record($record, $params=array()) {
		global $adb, $table_prefix;
		if (empty($adb) || !Vtiger_Utils::CheckTable($table_prefix.'_processmaker_rec')) return false;
		static $cache = array();
	...
	function get_all_related_modules($related_modules){
		...
		$modules_not_supported = array("Sms", "Messages", "Fax", "Processes");
