...
VTE.Homestuff = VTE.Homestuff || {
	 ...
	 */
	fetch_homeDB: function(stuffid) {
		var me = VTE.Homestuff;
		me.showBusy(stuffid);
		jQuery.ajax({
			url: 'index.php',
			method: 'POST',
			data: 'module=Dashboard&action=DashboardAjax&file=HomepageDB',
			success: function(result) {
				jQuery('#stuffcont_' + stuffid).hide();
				jQuery('#stuffcont_' + stuffid).html(result);
				me.hideBusy(stuffid);
				jQuery('#stuffcont_' + stuffid).fadeIn(); // crmv@168103
			}
		});
	},
...
function fetch_homeDB(stuffid) {
	return VTE.callDeprecated('fetch_homeDB', VTE.Homestuff.fetch_homeDB, arguments);
}
