...
if (VteSession::hasKey('pbls_lead_sources') && count(VteSession::get('pbls_lead_sources')) > 0 && !isset($_REQUEST['pbls_lead_sources'])) {
	$tempx = VteSession::get('pbls_lead_sources');
	$log->debug("_SESSION['pbls_lead_sources'] is:");
	$log->debug(VteSession::get('pbls_lead_sources'));
}
...
if (VteSession::hasKey('pbls_ids') && count(VteSession::get('pbls_ids')) != 0 && !isset($_REQUEST['pbls_ids'])) {
	$ids = VteSession::get('pbls_ids');
	$log->debug("_SESSION['pbls_ids'] is:");
	$log->debug(VteSession::get('pbls_ids'));
}
...
<td valign='top'><select name="pbls_lead_sources[]" multiple size='3'><?php echo get_select_options_with_id($comboFieldArray['lead_source_dom'],VteSession::get('pbls_lead_sources')); ?></select></td>
</tr><tr>
...
<?php if($is_admin==false && $profileGlobalPermission[2] == 1 && ($defaultOrgSharingPermission[getTabid('Potentials')] == 3 or $defaultOrgSharingPermission[getTabid('Potentials')] == 0)) { ?>
	<td valign='top'><select name="pbls_ids[]" multiple size='3'><?php echo get_select_options_with_id(get_user_array(FALSE, "Active", $current_user->id,'private'),VteSession::get('pbls_ids')); ?></select></td>
<?php } else { ?>
	<td valign='top'><select name="pbls_ids[]" multiple size='3'><?php echo get_select_options_with_id(get_user_array(false, "Active" ,$current_user->id),VteSession::get('pbls_ids')); ?></select></td>
<?php } ?>		
</tr><tr>
