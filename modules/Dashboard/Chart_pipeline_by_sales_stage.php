...
{
	VteSession::set('pbss_date_start', ""); 
}
elseif(isset($_REQUEST['pbss_date_start']) && $_REQUEST['pbss_date_start'] != '')
	VteSession::set('pbss_date_start', $_REQUEST['pbss_date_start']);
if(isset($_REQUEST['pbss_date_end']) && $_REQUEST['pbss_date_end'] == '')
{
	VteSession::set('pbss_date_end', ""); 
}
elseif(isset($_REQUEST['pbss_date_start']) && $_REQUEST['pbss_date_end'] != '')
	VteSession::set('pbss_date_end', $_REQUEST['pbss_date_end']);
//get the dates to display
if (VteSession::hasKey('pbss_date_start') && VteSession::get('pbss_date_start') != '' && !isset($_REQUEST['pbss_date_start'])) {
	$date_start = VteSession::get('pbss_date_start');
	$log->debug("_SESSION['pbss_date_start'] is:");
	$log->debug(VteSession::get('pbss_date_start'));
}
elseif (isset($_REQUEST['pbss_date_start']) && $_REQUEST['pbss_date_start'] != '') {
...
if (VteSession::hasKey('pbss_date_end') && VteSession::get('pbss_date_end') != '' && !isset($_REQUEST['pbss_date_end'])) {
	$date_end = VteSession::get('pbss_date_end');
	$log->debug("_SESSION['pbss_date_end'] is:");
	$log->debug(VteSession::get('pbss_date_end'));
}
...
if (VteSession::hasKey('pbss_sales_stages') && count(VteSession::get('pbss_sales_stages')) > 0 && !isset($_REQUEST['pbss_sales_stages'])) {
	$tempx = VteSession::get('pbss_sales_stages');
	$log->debug("_SESSION['pbss_sales_stages'] is:");
	$log->debug(VteSession::get('pbss_sales_stages'));
}
...
if (VteSession::hasKey('pbss_ids') && count(VteSession::get('pbss_ids')) != 0 && !isset($_REQUEST['pbss_ids'])) {
	$ids = VteSession::get('pbss_ids');
	$log->debug("_SESSION['pbss_ids'] is:");
	$log->debug(VteSession::get('pbss_ids'));
}
...
<script type="text/javascript" src="include/js/jscalendar/lang/calendar-<?php echo $cal_lang ?>.js"></script>
...
<td valign='top' ><input class="text" name="pbss_date_start" size='12' maxlength='10' id='date_start' value='<?php if (VteSession::hasKey('pbss_date_start')) echo VteSession::get('pbss_date_start')?>'>  <img src="<?php echo resourcever('calendar.gif') ?>" id="date_start_trigger"> </td>
</tr><tr>
<tr>
<td valign='top' nowrap><?php echo $current_module_strings['LBL_DATE_END'];?><br><em><?php echo $app_strings['NTC_DATE_FORMAT']?></em></td>
<td valign='top' ><input class="text" name="pbss_date_end" size='12' maxlength='10' id='date_end' value='<?php if (VteSession::hasKey('pbss_date_end')) echo VteSession::get('pbss_date_end')?>'>  <img src="<?php echo resourcever('calendar.gif') ?>" id="date_end_trigger"> </td>
</tr><tr>
<td valign='top' nowrap><?php echo $current_module_strings['LBL_SALES_STAGES'];?></td>
<td valign='top' ><select name="pbss_sales_stages[]" multiple size='3'><?php echo get_select_options_with_id($comboFieldArray['sales_stage_dom'],VteSession::get('pbss_sales_stages')); ?></select></td>
</tr><tr>
...
<?php if($is_admin==false && $profileGlobalPermission[2] == 1 && ($defaultOrgSharingPermission[getTabid('Potentials')] == 3 or $defaultOrgSharingPermission[getTabid('Potentials')] == 0)) { ?>
	<td valign='top'><select name="pbss_ids[]" multiple size='3'><?php echo get_select_options_with_id(get_user_array(FALSE, "Active", $current_user->id,'private'),VteSession::get('pbss_ids')); ?></select></td>
	<?php } else { ?>
	<td valign='top'><select name="pbss_ids[]" multiple size='3'><?php echo get_select_options_with_id(get_user_array(FALSE,"Active",$current_user->id),VteSession::get('pbss_ids')); ?></select></td>	
<?php } ?>		
</tr><tr>
