...
if (VteSession::hasKey('lsbo_lead_sources') && count(VteSession::get('lsbo_lead_sources')) > 0 && !isset($_REQUEST['lsbo_lead_sources'])) {
	$tempx = VteSession::get('lsbo_lead_sources');
	$log->debug("_SESSION['lsbo_lead_sources'] is:");
	$log->debug(VteSession::get('lsbo_lead_sources'));
}
...
if (VteSession::hasKey('lsbo_ids') && count(VteSession::get('lsbo_ids')) != 0 && !isset($_REQUEST['lsbo_ids'])) {
	$ids = VteSession::get('lsbo_ids');
	$log->debug("_SESSION['lsbo_ids'] is:");
	$log->debug(VteSession::get('lsbo_ids'));
}
...
<td valign='top'><select name="lsbo_lead_sources[]" multiple size='3'><?php echo get_select_options_with_id($comboFieldArray['lead_source_dom'],VteSession::get('lsbo_lead_sources')); ?></select></td>
</tr><tr>
...
<?php if($is_admin==false && $profileGlobalPermission[2] == 1 && ($defaultOrgSharingPermission[getTabid('Potentials')] == 3 or $defaultOrgSharingPermission[getTabid('Potentials')] == 0)) { ?>
	<td valign='top'><select name="lsbo_ids[]" multiple size='3'><?php echo get_select_options_with_id(get_user_array(FALSE, "Active", $current_user->id,'private'),VteSession::get('lsbo_ids')); ?></select></td>
<?php } else { ?>
	<td valign='top' ><select name="lsbo_ids[]" multiple size='3'><?php echo get_select_options_with_id(get_user_array(FALSE, "Active",$current_user->id),VteSession::get('lsbo_ids')); ?></select></td>
<?php } ?>	
</tr><tr>
