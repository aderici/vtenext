...
if (VteSession::hasKey('mypbss_date_start') && VteSession::get('mypbss_date_start') != '' && !isset($_REQUEST['mypbss_date_start'])) {
	$date_start = VteSession::get('mypbss_date_start');
	$log->debug("_SESSION['mypbss_date_start'] is:");
	$log->debug(VteSession::get('mypbss_date_start'));
}
...
if (VteSession::hasKey('mypbss_date_end') && VteSession::get('mypbss_date_end') != '' && !isset($_REQUEST['mypbss_date_end'])) {
	$date_end = VteSession::get('mypbss_date_end');
	$log->debug("_SESSION['mypbss_date_end'] is:");
	$log->debug(VteSession::get('mypbss_date_end'));
}
...
if (VteSession::hasKey('mypbss_sales_stages') && count(VteSession::get('mypbss_sales_stages')) > 0 && !isset($_REQUEST['mypbss_sales_stages'])) {
	$tempx = VteSession::get('mypbss_sales_stages');
	$log->debug("_SESSION['mypbss_sales_stages'] is:");
	$log->debug(VteSession::get('mypbss_sales_stages'));
}
...
<td valign='top' ><input class="text" name="mypbss_date_start" size='12' maxlength='10' id='date_start' value='<?php if (VteSession::hasKey('mypbss_date_start')) echo VteSession::get('mypbss_date_start')?>'>  <img src="<?php echo resourcever('calendar.gif') ?>" id="date_start_trigger"> </td>
</tr><tr>
<tr>
<td valign='top' nowrap><?php echo $current_module_strings['LBL_DATE_END'];?><br><em><?php echo $app_strings['NTC_DATE_FORMAT']?></em></td>
<td valign='top' ><input class="text" name="mypbss_date_end" size='12' maxlength='10' id='date_end' value='<?php if (VteSession::hasKey('mypbss_date_end')) echo VteSession::get('mypbss_date_end')?>'>  <img src="<?php echo resourcever('calendar.gif') ?>" id="date_end_trigger"> </td>
</tr><tr>

<td valign='top' nowrap><?php echo $current_module_strings['LBL_SALES_STAGES'];?></td>
<td valign='top' ><select name="mypbss_sales_stages[]" multiple size='5'><?php echo get_select_options_with_id($comboFieldArray['sales_stage_dom'],VteSession::get('mypbss_sales_stages')); ?></select></td>
</tr><tr>
