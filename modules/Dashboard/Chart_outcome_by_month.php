...
{
	VteSession::set('obm_date_start', ""); 
}
elseif(isset($_REQUEST['obm_date_start']) && $_REQUEST['obm_date_start'] != '')
	VteSession::set('obm_date_start', $_REQUEST['obm_date_start']);
if(isset($_REQUEST['obm_date_end']) && $_REQUEST['obm_date_end'] == '')
{
	VteSession::set('obm_date_end', ""); 
}
elseif(isset($_REQUEST['obm_date_end']) && $_REQUEST['obm_date_end'] != '')
	VteSession::set('obm_date_end', $_REQUEST['obm_date_end']);
if (VteSession::hasKey('obm_date_start') && VteSession::get('obm_date_start') != '' && !isset($_REQUEST['obm_date_start'])) {
	$date_start = VteSession::get('obm_date_start');
	$log->debug("_SESSION['obm_date_start'] is:");
	$log->debug(VteSession::get('obm_date_start'));
}
elseif (isset($_REQUEST['obm_date_start']) && $_REQUEST['obm_date_start'] != '') {
...
if (VteSession::hasKey('obm_date_end') && VteSession::get('obm_date_end') != '' && !isset($_REQUEST['obm_date_end'])) {
	$date_end = VteSession::get('obm_date_end');
	$log->debug("_SESSION['obm_date_end'] is:");
	$log->debug(VteSession::get('obm_date_end'));
}
elseif (isset($_REQUEST['obm_date_end']) && $_REQUEST['obm_date_end'] != '') {
...
if (VteSession::hasKey('obm_ids') && count(VteSession::get('obm_ids')) != 0 && !isset($_REQUEST['obm_ids'])) {
	$ids = VteSession::get('obm_ids');
	$log->debug("_SESSION['obm_ids'] is:");
	$log->debug(VteSession::get('obm_ids'));
}
...
<td valign='top' ><input class="text" name="obm_date_start" size='12' maxlength='10' id='date_start'  value='<?php if (VteSession::hasKey('obm_date_start')) echo VteSession::get('obm_date_start')?>'>  <img src="<?php echo resourcever('calendar.gif') ?>" id="date_start_trigger"> </td>
</tr><tr>
<tr>
<td valign='top' nowrap><?php echo $current_module_strings['LBL_DATE_END'];?><br><em><?php echo $app_strings['NTC_DATE_FORMAT']?></em></td>
<td valign='top' ><input class="text" name="obm_date_end" size='12' maxlength='10' id='date_end' value='<?php if (VteSession::hasKey('obm_date_end')) echo VteSession::get('obm_date_end')?>'>  <img src="<?php echo resourcever('calendar.gif') ?>" id="date_end_trigger"> </td>
</tr><tr>
...
<?php if($is_admin==false && $profileGlobalPermission[2] == 1 && ($defaultOrgSharingPermission[getTabid('Potentials')] == 3 or $defaultOrgSharingPermission[getTabid('Potentials')] == 0)) { ?>
	<td valign='top' ><select name="obm_ids[]" multiple size='3'><?php echo get_select_options_with_id(get_user_array(FALSE, "Active", $current_user->id,'private'),VteSession::get('obm_ids')); ?></select></td>
<?php } else { ?>
	<td valign='top' ><select name="obm_ids[]" multiple size='3'><?php echo get_select_options_with_id(get_user_array(FALSE, "Active",$current_user->id),VteSession::get('obm_ids')); ?></select></td>
<?php } ?>
</tr><tr>
