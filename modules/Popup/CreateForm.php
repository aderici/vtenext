...
if ($from_module == 'Messages' && !empty($from_crmid) && empty($relatedIds)) {
	...
	$email = $sugg['email'];
	$name = $sugg['name'];
	echo '<script type="text/javascript">jQuery(\'#relevant_ids\').val(\''.Zend_Json::encode($relatedIds).'\');jQuery(\'#original_email\').val(\''.$email.'\');jQuery(\'#original_name\').val(\''.$name.'\');</script>';
}
//crmv@168655e
