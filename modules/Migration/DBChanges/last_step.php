...
elseif($adb->isOracle()){
	$sql = "select table_name,table_type from cat where table_type in ('SEQUENCE') and table_name not like 'BIN\$%'";
	$res = $adb->query($sql);
	if ($res){
...
Migration_Utils::installBetaModules($migrationInfo['selected_beta_modules'], $migrationInfo['source_directory'], $migrationInfo['root_directory']); // crmv@151405
