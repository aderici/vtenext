...
class ModNotificationsHandler extends VTEventHandler {
	...
	function handleEvent($eventName, $data) {
		...
		if ($data->focus instanceof ModNotifications && $mode == 'create') {
			...
			if ($eventName == 'vtiger.entity.aftersave.notifications') { // crmv@198950
		...
		*/
		...
		if ($data->focus instanceof HelpDesk) {
			...
			if ($eventName == 'vtiger.entity.aftersave.notifications' && !in_array($column_fields['assigned_user_id'],$already_notified_users)) { // crmv@198950
				if ($mode == 'create') {
					$type = 'Ticket created';
				} else {
					$type = 'Ticket changed';
		...
		*/
		if ($data->focus instanceof Products) {
			...
			if ($eventName == 'vtiger.entity.aftersave.notifications' && $data->stock_level_check) { // crmv@198950
