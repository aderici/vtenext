...
$res = $adb->pquery("SELECT id FROM {$table_prefix}_pdfmaker_prodbloc_tpl WHERE id = ? AND name = ?", array(1, $name));
...
$res = $adb->pquery("SELECT id FROM {$table_prefix}_pdfmaker_prodbloc_tpl WHERE name = ?", array($name));
if ($res && $adb->num_rows($res) == 0) {
