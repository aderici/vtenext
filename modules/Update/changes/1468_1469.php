		...
		while ($row = $adb->FetchByAssoc($res, -1, false)) {
			...
			if ($notid > 0 && $relid > 0) {
				...
				if ($adb->isMysql()) {
					...
					// crmv@129940
					$res2 = $adb->pquery(
						"UPDATE {$table_prefix}_crmentity
						SET modifiedtime = ?, deleted = 1
						FROM {$table_prefix}_crmentity
						INNER JOIN ".$table_prefix."_modnotifications ON ".$table_prefix."_crmentity.crmid = ".$table_prefix."_modnotifications.modnotificationsid $setypeCond
						WHERE smownerid = ? AND related_to = ? AND modnotificationsid != ?",
						array($now, $userid, $relid, $notid)
					);
					// crmv@129940e
