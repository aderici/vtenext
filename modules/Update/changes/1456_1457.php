...
if (PerformancePrefs::getBoolean('CRMENTITY_PARTITIONED')) {
	$setypeCond = "AND c.setype = 'Products'";
}		
if ($adb->isMysql()) {
	$res = $adb->query(
	...
	if ($res && $adb->num_rows($res) > 0) {
		while ($row = $adb->fetchByAssoc($res, -1, false)) {
			$adb->pquery("UPDATE {$table_prefix}_crmentity c SET c.smownerid = ? WHERE c.crmid = ? $setypeCond", array($row['handler'], $row['productid']));
		}
	}	
}
...
for ($i=0; $i<4; ++$i) {
	if ($adb->isMysql()) {
		...
		$adb->pquery("DELETE FROM {$table_prefix}_org_share_action2tab WHERE share_action_id = ? AND tabid = ?", array($i, $tabid)); // crmv@129940
...
if (isModuleInstalled('Services')) {
	...
	$setypeCond = '';
	if (PerformancePrefs::getBoolean('CRMENTITY_PARTITIONED')) {
		$setypeCond = "AND c.setype = 'Services'";
	}		
	if ($adb->isMysql()) {
		$res = $adb->query(
			"UPDATE {$table_prefix}_crmentity c
		...
		if ($res && $adb->num_rows($res) > 0) {
			while ($row = $adb->fetchByAssoc($res, -1, false)) {
				$adb->pquery("UPDATE {$table_prefix}_crmentity c SET c.smownerid = ? WHERE c.crmid = ? $setypeCond", array($row['handler'], $row['serviceid']));
			}
		}	
	}
