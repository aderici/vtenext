...
class Newsletter extends CRMEntity {
	...
	function vtlib_handler($modulename, $event_type) {
		if($event_type == 'module.postinstall') {
			...
			Vtiger_Link::addLink($newsletterModule->id,'HEADERSCRIPT','StatisticsScript','modules/Campaigns/Statistics.js');
	...
	function getTargetElements($targetid,&$return,&$return_for_modules) {
		...
		if (!empty($ids)) {
			...
			foreach ($ids as $tid) {
				$this->getTargetElements($targetid,$return,$return_for_modules);
			}
		}
