...
<table border="0" cellpadding="2" cellspacing="0" width="100%" class="small" style="padding-top:5px">
	...
	<tr>
		...
		<td>
			...
			<div class="dvtCellInfo" style="margin-left:5px; float:left">
				<span id="task-emailfieldsreplyto-busyicon"><b>{'LBL_LOADING'|getTranslatedString:'com_vtiger_workflow'}</b>{include file="LoadingIndicator.tpl"}</span>
				<select id="task-emailfieldsreplyto" class="detailedViewTextBox notdropdown populateField" style="display: none;"><option value=''>{'LBL_SELECT_OPTION_DOTDOTDOT'|getTranslatedString:'com_vtiger_workflow'}</option></select>
			</div>
			{* crmv@106857 *}
			{assign var="target_mode" value="append_input_comma"}
	...
	<tr>
		...
		<td>
			...
			{foreach key=k item=i from=$INVOLVED_RECORD_PICK}
				{if isset($i.group)}
