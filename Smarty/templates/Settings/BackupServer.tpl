...
<tbody><tr>
		...
		{include file='Buttons_List.tpl'} {* crmv@30683 *}
				...
				<tr>
					<td width=50 rowspan=2 valign=top><img src="{'backupserver.gif'|resourcever}" alt="{$MOD.LBL_USERS}" width="48" height="48" border=0 title="{$MOD.LBL_USERS}"></td>
...
function clearBackupServer(Obj) {
	jQuery.ajax({
		url: 'index.php',
		method: 'post',
		data: 'module=Settings&action=SettingsAjax&ajax=true&file=BackupServerConfig&opmode=del',
		success: function(result) {
			jQuery("#BackupServerContents").html(result);
		}
	});
	backupenabled(Obj);	
}
...
function backupenabled(ochkbox)
{
	...
	if(ochkbox.checked == true) {
		jQuery('#bckcontents').show();
		var status='enabled';
		jQuery('#view_info').html(alert_arr.MSG_FTP_BACKUP_ENABLED+', ' + alert_arr.MSG_CONFIRM_FTP_DETAILS);
		jQuery('#view_info').show();
		jQuery("#ftp_buttons").show();

		jQuery.ajax({
			url: 'index.php',
			method: 'post',
			data: 'module=Settings&action=SettingsAjax&file=SaveEnableBackup&ajax=true&GetBackupDetail=true&servertype=ftp_backup',
			success: function(result) {
	...
	{
		jQuery('#bckcontents').hide();
		var status = 'disabled';
		jQuery('#view_info').html(alert_arr.MSG_FTP_BACKUP_DISABLED);
		jQuery('#view_info').show();
		jQuery("#ftp_buttons").hide();
	}
	
	jQuery.ajax({
		url: 'index.php',
		method: 'post',
		data: 'module=Settings&action=SettingsAjax&file=SaveEnableBackup&ajax=true&enable_ftp_backup='+status,
		success: function(result) {
			jQuery("#status").hide();
		}
	});
...
function localbackupenabled(ochkbox)
{
	...
	if(ochkbox.checked == true)
		{
				jQuery('#localbackup_buttons').show()
				jQuery('#localbackup_fields').show();
				var status='enabled';
				jQuery('#view_info').html(alert_arr.MSG_LOCAL_BACKUP_ENABLED+', '+alert_arr.MSG_CONFIRM_PATH);
				jQuery('#view_info').show();
				jQuery.ajax({
					url: 'index.php',
					method: 'post',
					data: 'module=Settings&action=SettingsAjax&file=SaveEnableBackup&ajax=true&GetBackupDetail=true&servertype=local_backup',
					success: function(result) {
									if(result.indexOf('FAILURE') > -1)
									{
		...
		{
				jQuery('#localbackup_buttons').hide();
				jQuery('#localbackup_fields').hide();
				var status = 'disabled';
				j§Query('#view_info').html(alert_arr.MSG_LOCAL_BACKUP_DISABLED);
				jQuery('#view_info').show();
		}
		jQuery.ajax({
			url: 'index.php',
			method: 'post',
			data: 'module=Settings&action=SettingsAjax&file=SaveEnableBackup&ajax=true&enable_local_backup='+status,
			success: function(result) {
				jQuery("#status").hide();
			}
		});	
