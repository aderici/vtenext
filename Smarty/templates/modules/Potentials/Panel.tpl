...
{* crmv@44187 crmv@45699 crmv@53923 crmv@191138 *}
		...
		<tbody>
			{foreach item=contact from=$ACCOUNT_CONTACTS}
				<tr id="potPanelContRow_{$contact.contactid}">
					<td class="text-nowrap"><span class="potPanelDisplayValue"><a href="javascript:potPanelEditContactRow('{$contact.contactid}')"><i class="vteicon" title="{$APP.LBL_EDIT_BUTTON}">edit</i></a></span></td>
					...
					<td><a href="javascript:;" onclick="startCall('{$contact.phone}', '{$contact.contactid}');">{$contact.phone}</a></td>
					<td>
	...
	<table class="vtetable">
		...
		<tbody>
			{foreach item=contact from=$OTHER_CONTACTS}
				<tr id="potPanelContRow_{$contact.contactid}" >
					<td class="text-nowrap"><span class="potPanelDisplayValue"><a href="javascript:potPanelEditContactRow('{$contact.contactid}')"><i class="vteicon" title="{$APP.LBL_EDIT_BUTTON}">edit</i></a> <a href="javascript:void(0)" onclick="potPanelDeleteContactRow('{$ID}', '{$contact.contactid}', 'Contacts', this)"><i class="vteicon" title="{$APP.LBL_DELETE_BUTTON}">delete</i></a></span></td>
					<td><a href="index.php?module=Contacts&action=DetailView&record={$contact.contactid}">{$contact.firstname} {$contact.lastname}</a></td>
					<td><a href="javascript:InternalMailer('{$contact.contactid}', '{$contact.email_fieldid}', 'email', 'Contacts', 'record_id')">{$contact.email}</a></td>
					<td><a href="javascript:;" onclick="startCall('{$contact.phone}', '{$contact.contactid}');">{$contact.phone}</a></td>
					<td>
		...
		<tbody>
			{foreach item=partner from=$PARTNERS}
				<tr id="potPanelContRow_{$partner.accountid}">
					<td class="text-nowrap"><span class="potPanelDisplayValue"><a href="javascript:potPanelEditContactRow('{$partner.accountid}')"><i class="vteicon" title="{$APP.LBL_EDIT_BUTTON}">edit</i></a> <a href="javascript:void(0)" onclick="potPanelDeleteContactRow('{$ID}', '{$partner.accountid}', 'Accounts', this)"><i class="vteicon" title="{$APP.LBL_DELETE_BUTTON}">delete</i></a></span></td>
					...
					<td><a href="javascript:;" onclick="startCall('{$partner.phone}', '{$partner.accountid}');">{$partner.phone}</a></td>
					<td>
						<span class="potPanelDisplayValue">
		...
		<tbody>
			{foreach item=compet from=$COMPETITORS}
				<tr id="potPanelContRow_{$compet.accountid}">
					...
					<td><a href="javascript:;" onclick="startCall('{$compet.phone}', '{$compet.accountid}');">{$compet.phone}</a></td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
					<td align="right">
						<span class="potPanelEditCont">
			...
			<tbody>
				{foreach item=line from=$PRODLINES.list}
					<tr>
						...
						{else}
							<td><a href="javascript:;">{$line.linename}</a></td>
						{/if}
