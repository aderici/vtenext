			...
			<span class="dvHeaderText {if $LAYOUT_CONFIG['record_title_inline'] eq 1}dvHeaderTextInline{else}dvHeaderTextMultiLine{/if}" data-record-inline="{$LAYOUT_CONFIG['record_title_inline']}">
				<div class="recordTitleName">
					...
					{if $SHOW_RECORD_NUMBER eq true}
						[ {$USE_ID_VALUE} ]&nbsp;
					{/if}
					<span class="recordName" title="{$NAME}">{$NAME}&nbsp;</div>
				</div>	
			 		...
			 		{if $smarty.request.module eq 'Calendar'}
			 			{if $ACTIVITY_MODE neq 'Task'}
							...
							{if $OP_MODE eq 'create_view' && !empty($RETURN_ID) && !empty($RETURN_MODULE)}
								...
								<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accesskey="{$APP.LBL_SAVE_BUTTON_KEY}" class="crmbutton save success" name="button" value="{$APP.LBL_SAVE_BUTTON_LABEL}" onclick="this.form.return2detail.value='yes'; this.form.action.value='Save'; VtigerJS_DialogBox.block(); jQuery(this.form).submit();" type="button" />
							{else}
			 					<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accesskey="{$APP.LBL_SAVE_BUTTON_KEY}" class="crmbutton save success" name="button" value="{$APP.LBL_SAVE_BUTTON_LABEL}" onclick="this.form.action.value='Save'; VtigerJS_DialogBox.block(); jQuery(this.form).submit();" type="button" />
			 				{/if}
			 			{else}
							{if $OP_MODE eq 'create_view' && !empty($RETURN_ID) && !empty($RETURN_MODULE)}
								<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accesskey="{$APP.LBL_SAVE_BUTTON_KEY}" class="crmbutton save success backButtonSave" name="button" value="{$APP.LBL_SAVE_AND_BACK_BUTTON_LABEL}" onclick="this.form.action.value='Save'; VtigerJS_DialogBox.block(); jQuery(this.form).submit();" type="button" />
								<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accesskey="{$APP.LBL_SAVE_BUTTON_KEY}" class="crmbutton save success" name="button" value="{$APP.LBL_SAVE_BUTTON_LABEL}" onclick="this.form.return2detail.value='yes'; this.form.action.value='Save'; VtigerJS_DialogBox.block(); jQuery(this.form).submit();" type="button" />
							{else}
			 					<input title="{$APP.LBL_SAVE_BUTTON_TITLE}" accesskey="{$APP.LBL_SAVE_BUTTON_KEY}" class="crmbutton save success" name="button" value="{$APP.LBL_SAVE_BUTTON_LABEL}" onclick="this.form.action.value='Save'; VtigerJS_DialogBox.block(); jQuery(this.form).submit();" type="button" />
			 				{/if}
