		...
		foreach($filelist as $filename=>$fileinfo) {
			...
			if(count($matches)) {
				...
				if($this->isLanguageType()) {
					...
					if (empty($error) && !$this->checkPicklistDuplicates($this->_modulexml, Vtiger_Module::getInstance($modulename))) {
						$error = getTranslatedString('LBL_IMPORT_PICKLIST_DUPLICATES_ERROR', 'Settings');
					}
					// crmv@195213e
					// crmv@197127
					if (empty($error) && !$this->checkModlights($this->_modulexml, Vtiger_Module::getInstance($modulename))) {
