...
class Vtiger_PackageExport {
	...
	function export_Info($moduleInstance) {
		...
		$result = $adb->pquery("SELECT * FROM {$table_prefix}_tab_info WHERE tabid = ? and prefname not in (?,?)", array($moduleInstance->id,'vtiger_min_version','vtiger_max_version'));
	...
	function export_Modlights($moduleInstance) {
		...
		if ($result && $adb->num_rows($result) > 0) {
			...
			while($row=$adb->fetchByAssoc($result)) {
				...
				$this->export_Module(Vtiger_Module::getInstance($modulelightname));
			}
